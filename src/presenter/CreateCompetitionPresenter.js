...
class CreateCompetitionPresenter{
    constructor(view){
        this.view = view;
        this.model = null; 
    }
    ...
    async createCompetition(name, type, startDate, endDate, target, details, friendList, userID){
        try{
            const isNumber = /^\d+$/;
            ...
            if (name === ''){
                ...
                if(type === 1){
                    ...
                    if(isNumber.test(target) === false){
                        ...
                        if(target <= 0){
                            ...
                            await this.model.createCompetition(name, type, startDate, next2Week, target, details, friendList, userID);
                        }
                ...
                if (type === 2){
                    ...
                    if (startDate >= endDate){
                        ...
                        this.model = new Competition();
                        await this.model.createCompetition(name, type, startDate, endDate, target, details, friendList, userID);
                    }
...
export default CreateCompetitionPresenter;